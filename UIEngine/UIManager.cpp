...
#if !defined(UI_BUILD_FOR_WIN32) || defined(UI_BUILD_FOR_WINCE)
CStdPtrArray CPaintManagerUI::m_aCustomFonts;
//CStdStringPtrMap CPaintManagerUI::m_mImageHash;
CStdStringPtrMap CPaintManagerUI::m_DefaultAttrHash;

DWORD CPaintManagerUI::m_dwDefaultDisabledColor = 0xFFA7A6AA;
DWORD CPaintManagerUI::m_dwDefaultFontColor = 0xFF000000;
DWORD CPaintManagerUI::m_dwDefaultLinkFontColor = 0xFF0000FF;
DWORD CPaintManagerUI::m_dwDefaultLinkHoverFontColor = 0xFFD3215F;
DWORD CPaintManagerUI::m_dwDefaultSelectedBkColor = 0xFFBAE4FF;
TFontInfo CPaintManagerUI::m_DefaultFontInfo;
#endif

CPaintManagerUI::CPaintManagerUI() :
m_hWndPaint(NULL),
m_hPreCaptureWindow(NULL),
m_hDcPaint(NULL),
...
m_bUsedAsUIDesigner(false),
m_bAssertWhenCustomDefinedNull(true),
#endif
...
#else
	if (m_DefaultFontInfo.hFont == NULL)
	{
		LOGFONT lf = { 0 };
		lf.lfWeight = FW_BOLD;
...
#ifdef UI_BUILD_FOR_DESIGNER
bool CPaintManagerUI::IsAssertWhenCustomDefinedNull() const
{
	return m_bAssertWhenCustomDefinedNull;
}

void CPaintManagerUI::SetAssertWhenCustomDefinedNull(bool bAsert)
{
	m_bAssertWhenCustomDefinedNull = bAsert;
}

bool CPaintManagerUI::IsUsedAsUIDesigner()
{
	return m_bUsedAsUIDesigner;
}

void CPaintManagerUI::SetUsedAsUIDesigner(bool bUIDesigner)
{
	m_bUsedAsUIDesigner = bUIDesigner;
}
#endif
...
CStdString CPaintManagerUI::GetResourcePath()
{
    return m_pStrResourcePath;
}

CStdString CPaintManagerUI::GetResourceZip()
{
...
#if defined(USING_XUNZIP)
		m_hResourceZip = (HANDLE)OpenZip((void*)pVoid, len, 3);
#else
		m_hResourceZip = (HANDLE)OpenZip((void*)pVoid, len, NULL);
#endif
}
...
#else
		m_hResourceZip = (HANDLE)OpenZip(sFile.GetData(), NULL);
#endif
	}
}

void CPaintManagerUI::UnzipResource()
{
	if( !GetResourceZip().IsEmpty() )
	{
		CStdString sFile = GetResourcePath() + GetResourceZip();
		HZIP hz = OpenZip(sFile.GetData(), NULL);
		if( hz == INVALID_HANDLE_VALUE )
		{
			return;
		}
		ZRESULT zr;
		zr = SetUnzipBaseDir(hz, GetResourcePath());
		if( zr != ZR_OK )
		{
			CloseZip(hz);
			return;
		...
		{   
			ZIPENTRY zeItem;
			zr = GetZipItem(hz, i, &zeItem);			
			zr = UnzipItem(hz, i, zeItem.name);
			if( zr != ZR_OK )
			{
				CloseZip(hz);
				return;
...
int CPaintManagerUI::GetOpacity() const
{
	return m_nOpacity;
}
	...
	case WM_PAINT:
		{
			...
			if( m_bUpdateNeeded ) {
				...
				if( !::IsRectEmpty(&rcClient) ) {
					...
					if( m_bFirstLayout ) {
                        ...
                        SendNotify(m_pRoot, kWindowInit,  0, 0, false);
					}
				}
			}
...
#endif
			{
				...
				SkWinWindow* pfWinWindow= reinterpret_cast<SkWinWindow*>(lParam);
				if (pfWinWindow)
					m_pRoot->DoPaint(pfWinWindow, ps.rcPaint);
#endif
...
#elif defined(UI_BUILD_FOR_SKIA)
					if (pfWinWindow)
						pPostPaintControl->DoPostPaint(pfWinWindow, ps.rcPaint);
#endif
				}
				...
				if( m_bShowUpdateRect ) {
					...
					if (pfWinWindow) {
						LOGPEN lp = {0};
						GetObject(m_hUpdateRectPen, sizeof(LOGPEN), &lp);

						SkPaint paint;
						paint.setStyle(SkPaint::kStroke_Style);
						paint.setColor(0xFFFF0000);
						paint.setStrokeWidth(lp.lopnWidth.x + 1);
						SkRect rect = SkRect::MakeXYWH(rcPaint.left, rcPaint.top, rcPaint.right - rcPaint.left, rcPaint.bottom - rcPaint.top);
	...
	case WM_PRINTCLIENT:
		{
			...
			SkWinWindow* pfWinWindow= reinterpret_cast<SkWinWindow*>(lParam);
			if (pfWinWindow)
				m_pRoot->DoPaint(pfWinWindow, rcClient);
#endif
...
#ifdef UI_BUILD_FOR_DESIGNER
			if (IsUsedAsUIDesigner() && !((GetKeyState(VK_CONTROL) < 0) || (GetKeyState(VK_SHIFT) < 0)))
				RemoveAllPitchUpon();
#endif
	...
	case WM_CTLCOLOREDIT:
		{
			...
			CWindowWnd* pThis = reinterpret_cast<CWindowWnd*>(::GetProp(hwndEdit, _T("WndX")));
			if (pThis != NULL)
			{
				CEditWnd* pEditWnd = reinterpret_cast<CEditWnd*>(pThis);
				if ((pEditWnd != NULL) && (pEditWnd->GetBkColor() != 0))
				{
					DWORD dwTextColor = pEditWnd->GetEditTextColor();
					DWORD dwBkColor = pEditWnd->GetBkColor();
					::SetTextColor(hDC, RGB(GetBValue(dwTextColor),GetGValue(dwTextColor),GetRValue(dwTextColor)));
					::SetBkColor(hDC, RGB(GetBValue(dwBkColor),GetGValue(dwBkColor),GetRValue(dwBkColor)));
					lRes = (LRESULT)::CreateSolidBrush(RGB(GetBValue(dwBkColor),GetGValue(dwBkColor),GetRValue(dwBkColor)));
				}
				else
				{
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
	...
	if( m_pFocus != NULL ) 
	{
		...
		event.dwTimestamp = ::GetTickCount();
		m_pFocus->Event(event);
		SendNotify(m_pFocus, kKillFocus);
		m_pFocus = NULL;
	}
...
{
	m_hPreCaptureWindow = ::SetCapture(m_hWndPaint);
...
void CPaintManagerUI::ReleaseCapture()
{
	...
	if (m_hPreCaptureWindow != NULL)
		::SetCapture(m_hPreCaptureWindow);
}
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, CControlUI* pControl, LPCTSTR type, DWORD mask)
{
	if (pControl == NULL)
		return NULL;
	...
	TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
	if( !data ) {
		if( AddImage(bitmap, pControl, type, mask) ) {
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, CControlUI* pControl, LPCTSTR type, DWORD mask)
{
	if (pControl == NULL)
		return NULL;
	...
	if( type != NULL ) {
		if( isdigit(*bitmap) ) {
			LPTSTR pstr = NULL;
			int iIndex = _tcstol(bitmap, &pstr, 10);
			data = CRenderEngine::LoadImage(iIndex, pControl, this, type, mask);
		}
	}
	else {
		data = CRenderEngine::LoadImage(bitmap, pControl, this, type, mask);
	}
            ...
            if( data != NULL ) {
                if( !data->sResType.IsEmpty() ) {
                    if( isdigit(*bitmap) ) {
                        LPTSTR pstr = NULL;
                        int iIndex = _tcstol(bitmap, &pstr, 10);
                        pNewData = CRenderEngine::LoadImage(iIndex, this, data->sResType.GetData(), data->dwMask);
                    }
                }
                else {
...
#ifdef UI_BUILD_FOR_DESIGNER
CControlUI* CPaintManagerUI::FindPitchUponControl()
{
	ASSERT(m_pRoot);
	return m_pRoot->FindControl(__FindSinglePitchUponControl, NULL, UIFIND_VISIBLE);
}

int CPaintManagerUI::FindPitchUponControlCount()
{
	int nCount = 0;
	ASSERT(m_pRoot);
	m_pRoot->FindControl(__FindCountOfPitchUponControl, &nCount, UIFIND_VISIBLE);
	return nCount;
}

bool CPaintManagerUI::FinAllPitchUponControl(CStdPtrArray* pControls)
{
	if (pControls != NULL)
	{
		ASSERT(m_pRoot);
		m_pRoot->FindControl(__FindAllPitchUponControl, pControls, UIFIND_VISIBLE);
	}
...
#ifdef UI_BUILD_FOR_DESIGNER
// 查找被选中的控件
CControlUI* CALLBACK CPaintManagerUI::__FindSinglePitchUponControl(CControlUI* pThis, LPVOID pData)
{
	return pThis->IsPitchUpon() ? pThis : NULL;
}

// 查找被选中控件的数目
CControlUI* CALLBACK CPaintManagerUI::__FindCountOfPitchUponControl(CControlUI* pThis, LPVOID pData)
{
	if (pThis->IsPitchUpon())
	{
		int* pnCount = static_cast<int*>(pData);
		(*pnCount)++;
	}
	return NULL;  // search all controls
}

CControlUI* CALLBACK CPaintManagerUI::__FindAllPitchUponControl(CControlUI* pThis, LPVOID pData)
{
	if (pThis->IsPitchUpon())
	{
		CStdPtrArray* pControls = static_cast<CStdPtrArray*>(pData);
		pControls->Add(pThis);
	}
	return NULL;  // search all controls
}

void CPaintManagerUI::RemoveAllPitchUpon(bool bOnlyRemovePitchUponTwice)
{
	if (m_pRoot)
	{
		if (bOnlyRemovePitchUponTwice)
		{
			m_pRoot->SetPitchUponContinousTwice(false);
		}
		else
		{
			m_pRoot->SetPitchUpon(false);
			m_pRoot->SetPitchUponContinousTwice(false);
		}
	}
}

void CPaintManagerUI::MoveAllPitchUponControls(LONG xOffset, LONG yOffset)
{
	if (m_pRoot)
		m_pRoot->Move(xOffset, yOffset);
}
#endif

#ifdef UI_BUILD_FOR_SKIA
SkWinWindow* CPaintManagerUI::GetSkiaContext() const
{
	return fWinWindow;
}

void CPaintManagerUI::SetSkiaContext(SkWinWindow* ctx)
{
	fWinWindow = ctx;
}
