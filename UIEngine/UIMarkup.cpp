...
{
#ifdef _UNICODE
	if (XMLFILE_ENCODING_AUTODETECT == encoding)
	{
		if ( dwSize >= 2 && ( ( pByte[0] == 0xFE && pByte[1] == 0xFF ) || ( pByte[0] == 0xFF && pByte[1] == 0xFE ) ) )
		{
			m_pstrXML = static_cast<LPTSTR>(calloc(dwSize + 1, sizeof(TCHAR)));
			ZeroMemory(m_pstrXML, (dwSize + 1)*sizeof(TCHAR));
			memcpy(m_pstrXML, pByte, dwSize * sizeof(TCHAR) );

			int nLen = static_cast<int>(dwSize / 2);
			m_pstrXML[nLen] = cNullTerminated;
		}
		else
		{
			...
			m_pstrXML[nWide] = cNullTerminated;
		}
	}
	else if (encoding == XMLFILE_ENCODING_UTF8)
	{
    ...
    else
    {
        if ( dwSize >= 2 && ( ( pByte[0] == 0xFE && pByte[1] == 0xFF ) || ( pByte[0] == 0xFF && pByte[1] == 0xFE ) ) )
        {
            ...
            m_pstrXML[dwSize] = cNullTerminated;
    ...
    if (encoding == XMLFILE_ENCODING_UTF8)
    {
        ...
        m_pstrXML[wide] = cNullTerminated;
...
{
	if ((pstrFilename == NULL) || (_tcslen(pstrFilename) <= 0) || (pManager == NULL))
	{
		return false;
	}
	bool ret = false;
	...
	tString tstrPath = pstrFilename;
	if (tstrPath.find(pManager->GetResourcePath()) == tString::npos)
	{
		tstrPath = pManager->GetResourcePath() + pstrFilename;
	}
	...
	if(pManager->GetResourceZip().IsEmpty() ) {
		...
		HANDLE hFile = ::CreateFile(tstrPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		ret = LoadFromMem(pByte, dwSize, encoding);
		delete[] pByte;
	}
	else {
		CStdString sFile = pManager->GetResourcePath() + pManager->GetResourceZip();
		HZIP hz = NULL;
...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        if( *pstrText == cNullTerminated && iParent <= 1 ) return true;
        _SkipWhitespace(pstrText);
		
		// utf8 convert to unicode, remove the unicode BOM(0xFEFF)
		if (*pstrText == 0xFEFF)
			++pstrText;

        if( *pstrText != _T('<') ) return _Failed(_T("Expected start tag"), pstrText);
        if( pstrText[1] == _T('/') ) return true;
        *pstrText++ = cNullTerminated;
        _SkipWhitespace(pstrText);
        // Skip comment or processing directive
        if( *pstrText == _T('!') || *pstrText == _T('?') ) {
            TCHAR ch = *pstrText;
            if( *pstrText == _T('!') ) ch = _T('-');
            while( *pstrText != cNullTerminated && !(*pstrText == ch && *(pstrText + 1) == _T('>')) ) pstrText = ::CharNext(pstrText);
            if( *pstrText != cNullTerminated ) pstrText += 2;
            _SkipWhitespace(pstrText);
            continue;
        }
        _SkipWhitespace(pstrText);
        ...
        if( *pstrText == cNullTerminated ) return _Failed(_T("Error parsing element name"), pstrText);
        // Parse attributes
...
{
    while( *pstr > cNullTerminated && *pstr <= _T(' ') ) pstr = ::CharNext(pstr);
}

/*
void CMarkup::_SkipWhitespace(LPTSTR& pstr) const
{
	while( *pstr != cNullTerminated && *pstr <= _T(' ') && *pstr > cNullTerminated ) pstr++;
}
*/
...
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != cNullTerminated && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}
...
{   
    ...
    *pstrText++ = cNullTerminated;
    _SkipWhitespace(pstrText);
    while( *pstrText != cNullTerminated && *pstrText != _T('>') && *pstrText != _T('/') ) {
        _SkipIdentifier(pstrText);
        LPTSTR pstrIdentifierEnd = pstrText;
        _SkipWhitespace(pstrText);
        if( *pstrText != _T('=') ) return _Failed(_T("Error while parsing attributes"), pstrText);
        *pstrText++ = _T(' ');
        *pstrIdentifierEnd = cNullTerminated;
        _SkipWhitespace(pstrText);
        if( *pstrText++ != _T('\"') ) return _Failed(_T("Expected attribute value"), pstrText);
        LPTSTR pstrDest = pstrText;
        if( !_ParseData(pstrText, pstrDest, _T('\"')) ) return false;
        if( *pstrText == cNullTerminated ) return _Failed(_T("Error while parsing attribute string"), pstrText);
        *pstrDest = cNullTerminated;
...
{
    while( *pstrText != cNullTerminated && *pstrText != cEnd ) {
		if( *pstrText == _T('&') ) {
			while( *pstrText == _T('&') ) {
