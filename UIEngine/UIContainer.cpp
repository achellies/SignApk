...
{
    return kContainerUIClassName;
}

LPVOID CContainerUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kIContainerUIInterfaceName) == 0 ) return static_cast<IContainerUI*>(this);
    else if( _tcsicmp(pstrName, kContainerUIInterfaceName) == 0 ) return static_cast<CContainerUI*>(this);
...
int CContainerUI::FindSelectable(int iIndex, bool bForward /*= true*/) const
{
    ...
    if( bForward ) {
        for( int i = iIndex; i < GetCount(); i++ ) {
            if( GetItemAt(i)->GetInterface(kIListItemUIInterfaceName) != NULL 
...
void CContainerUI::DoPaint(void* ctx, const RECT& rcPaint)
{
	if (ctx == NULL) return;
	...
	CRenderClip clip;
	CRenderClip::GenerateClip(ctx, rcTemp, clip);
		...
		if( !::IntersectRect(&rcTemp, &rcPaint, &rc) ) {
			for( int it = 0; it < m_items.GetSize(); it++ ) {
				...
				if( pControl ->IsFloat() ) {
					...
					pControl->DoPaint(ctx, rcPaint);
				}
			}
		}
		else {
			CRenderClip childClip;
			CRenderClip::GenerateClip(ctx, rcTemp, childClip);
			for( int it = 0; it < m_items.GetSize(); it++ ) {
				CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
				if( !pControl->IsVisible() ) continue;
				if( !::IntersectRect(&rcTemp, &rcPaint, &pControl->GetPos()) ) continue;
				if( pControl ->IsFloat() ) {
					if( !::IntersectRect(&rcTemp, &m_rcItem, &pControl->GetPos()) ) continue;
					CRenderClip::UseOldClipBegin(ctx, childClip);
					pControl->DoPaint(ctx, rcPaint);
					CRenderClip::UseOldClipEnd(ctx, childClip);
				}
				else {
    ...
    if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() ) {
        if( ::IntersectRect(&rcTemp, &rcPaint, &m_pVerticalScrollBar->GetPos()) ) {
            m_pVerticalScrollBar->DoPaint(ctx, rcPaint);
        }
    }

    if( m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsVisible() ) {
        if( ::IntersectRect(&rcTemp, &rcPaint, &m_pHorizontalScrollBar->GetPos()) ) {
            m_pHorizontalScrollBar->DoPaint(ctx, rcPaint);
        }
    }
}

#ifdef UI_BUILD_FOR_DESIGNER
void CContainerUI::SetPitchUpon(bool bPitchUpon)
{
	if (!IsUsedForPitchUpon())
	{
		CControlUI::SetPitchUpon(false);
	}
	else
	{
		CControlUI::SetPitchUpon(bPitchUpon);

		for( int it = 0; it < m_items.GetSize(); it++ ) {
			CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
			pControl->SetPitchUpon(bPitchUpon);
		}
	}
}

void CContainerUI::SetPitchUponContinousTwice(bool bPitchUponContinousTwice)
{
	if (!IsUsedForPitchUpon())
	{
		CControlUI::SetPitchUponContinousTwice(false);
	}
	else
	{
		CControlUI::SetPitchUponContinousTwice(bPitchUponContinousTwice);

		for( int it = 0; it < m_items.GetSize(); it++ ) {
			CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
			pControl->SetPitchUponContinousTwice(bPitchUponContinousTwice);
		}
	}

}

void CContainerUI::Move(LONG xOffset, LONG yOffset)
{
	CControlUI::Move(xOffset, yOffset);

	for( int it = 0; it < m_items.GetSize(); it++ ) {
		CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
		pControl->Move(xOffset, yOffset);
	}
}
#endif
...
{
    return kVerticalLayoutUIClassName;
}

LPVOID CVerticalLayoutUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcsicmp(pstrName, kVerticalLayoutUIInterfaceName) == 0 ) return static_cast<CVerticalLayoutUI*>(this);
...
{
#ifdef UI_BUILD_FOR_DESIGNER
	if (IsUsedForPitchUpon() && (m_pManager != NULL) && m_pManager->IsUsedAsUIDesigner())
	{
...
{
    return kHorizontalLayoutUIClassName;
}

LPVOID CHorizontalLayoutUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kHorizontalLayoutUIInterfaceName) == 0 ) return static_cast<CHorizontalLayoutUI*>(this);
    return CContainerUI::GetInterface(pstrName);
}
...
void CHorizontalLayoutUI::DoPostPaint(void* ctx, const RECT& rcPaint)
{
	if (ctx == NULL) return;

    if( (m_uButtonState & UISTATE_CAPTURED) != 0 && !m_bImmMode ) {
        RECT rcSeparator = GetThumbRect(true);
        CRenderEngine::DrawColor(ctx, rcSeparator, 0xAA000000);
    }
}
...
{
    return kTileLayoutUIClassName;
}

LPVOID CTileLayoutUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kTileLayoutUIInterfaceName) == 0 ) return static_cast<CTileLayoutUI*>(this);
    return CContainerUI::GetInterface(pstrName);
}
...
{
    return kDialogLayoutUIClassName;
}

LPVOID CDialogLayoutUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kDialogLayoutUIInterfaceName) == 0 ) return static_cast<CDialogLayoutUI*>(this);
    return CContainerUI::GetInterface(pstrName);
}
...
{
    return kTabLayoutUIClassName;
}

LPVOID CTabLayoutUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kTabLayoutUIInterfaceName) == 0 ) return static_cast<CTabLayoutUI*>(this);
    return CContainerUI::GetInterface(pstrName);
}
