...
CControlUI* CMenuBuilderCallback::CreateControl(LPCTSTR pstrClass, CPaintManagerUI* pManager)
{
...
//
// CMenuWnd
INotifyUI* CMenuWnd::m_pNotifyUI = NULL;
...
void CMenuWnd::Init(CMenuElementUI* pOwner, INotifyUI* pNotifyUI, STRINGorID xml, LPCTSTR pSkinType, POINT point)
{
	m_BasedPoint = point;
    m_pOwner = pOwner;
    m_pLayout = NULL;
	m_pNotifyUI = pNotifyUI;
...
void CMenuWnd::OnFinalMessage(HWND hWnd)
{
	RemoveObserver();
	if( m_pOwner != NULL ) {
		m_pLayout->SetManager(m_pOwner->GetParent()->GetManager(), NULL, true);
...
void CMenuElementUI::DoPaint(void* ctx, const RECT& rcPaint)
{
	...
	for (int i = 0; i < GetCount(); ++i)
	{
		...
		{
			GetItemAt(i)->DoPaint(ctx, rcPaint);
		}
	}
}

void CMenuElementUI::DrawItemText(void* ctx, const RECT& rcItem)
{
...
SIZE CMenuElementUI::EstimateSize(SIZE szAvailable)
{
	...
	if(cXY.cy == 0) {
		...
		if( pInfo->bShowHtml ) {   
			int nLinks = 0;
			CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), this, m_pManager, rcText, m_sText, iTextColor, NULL, NULL, nLinks, DT_CALCRECT | pInfo->uTextStyle);
		}
		else {
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	m_pWindow->Init(static_cast<CMenuElementUI*>(this), CMenuWnd::m_pNotifyUI, _T(""), _T(""), CPoint());
}
