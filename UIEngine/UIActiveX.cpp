...
class CActiveXWnd : public CSkUIWindow
#endif
{
public:
...
#elif defined(UI_BUILD_FOR_SKIA)
LRESULT CActiveXWnd::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam)
#endif
{
    PAINTSTRUCT ps = { 0 };
...
{
    return kUIActiveXClassName;
}

LPVOID CActiveXUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, kUIActiveXInterfaceName) == 0 ) return static_cast<CActiveXUI*>(this);
...
void CActiveXUI::DoPaint(void* ctx, const RECT& rcPaint)
{
	if (ctx == NULL) return;
...
#endif // #if defined(UI_BUILD_FOR_WIN32) && !defined(UI_BUILD_FOR_WINCE)
