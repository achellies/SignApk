...
#include "..\third_party\RichEd20\src\textserv.h"
...
HRESULT CTxtWinHost::QueryInterface(REFIID riid, void **ppvObject)
{
    ...
    *ppvObject = NULL;
	...
	{
        AddRef();
        *ppvObject = static_cast<ITextHost2 *>(this);
        hr = S_OK;
	}
#endif
    return hr;
...
HRESULT CTxtWinHost::TxGetWindow(HWND *phwnd)
{
	...
	if ((m_re != NULL) && (phwnd != NULL))
	{
		*phwnd = m_re->GetManager()->GetPaintWindow();
		hr = S_OK;
	}
...
HRESULT CTxtWinHost::TxSetForegroundWindow()
{
	...
	if (m_re != NULL)
	{
		SetForegroundWindow(m_re->GetManager()->GetPaintWindow());
	}
...
{
    return kRichEditUIClassName;
}

LPVOID CRichEditUI::GetInterface(LPCTSTR pstrName)
{
    if( _tcsicmp(pstrName, kRichEditUIInterfaceName) == 0 ) return static_cast<CRichEditUI*>(this);
...
IDropTarget* CRichEditUI::GetTxDropTarget()
{
    ...
    if(m_pTwh &&  m_pTwh->GetTextServices()->TxGetDropTarget(&pdt) == NOERROR ) return pdt;
    return NULL;
}
...
void CRichEditUI::DoPaint(void* ctx, const RECT& rcPaint)
{
	if (ctx == NULL) return;
