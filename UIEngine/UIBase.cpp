...
{
    m_szBuffer[0] = cNullTerminated;
}

CStdString::CStdString(const TCHAR ch) : m_pstr(m_szBuffer)
{
    m_szBuffer[0] = ch;
    m_szBuffer[1] = cNullTerminated;
}
...
void CStdString::Assign(LPCTSTR pstr, int cchMax)
{
    ...
    m_pstr[cchMax] = cNullTerminated;
}

bool CStdString::IsEmpty() const 
{ 
    return m_pstr[0] == cNullTerminated;
}
