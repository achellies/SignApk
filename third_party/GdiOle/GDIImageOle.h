...
#ifndef _WIN32_WCE
	public ISimpleDataObjImpl<CGDIImageOle>,
#endif
...
public:
	HRESULT OnDraw(ATL_DRAWINFO& di)
	{
		m_pImage->Draw(di.hdcDraw,
			di.prcBounds->left,
			di.prcBounds->top,
			min(m_dwW, DWORD(di.prcBounds->right - di.prcBounds->left)),
			min(m_dwH, DWORD(di.prcBounds->bottom - di.prcBounds->top)), 0, 0, 0, 0,
			min(m_dwW, DWORD(di.prcBounds->right - di.prcBounds->left)),
			min(m_dwH, DWORD(di.prcBounds->bottom - di.prcBounds->top)));
	...
	*/
	...
	STDMETHOD(put_SetImage)(CGDIImage *pImage, LPCWSTR pStrData, HWND hCallbackWnd, DWORD dwUpdateMsg);
